...
**复杂度：** O(n)——每个节点后序访问一次；递归栈空间 O(h)，h 为树高。
...
**常见追问：**
- 双向 BFS——从两端同时搜，指数分枝减半。
- Word Ladder I（只要长度）——一轮 BFS 即可。
- 返最短路径个数而非所有路径。
- 除替换外还允许增/删——邻居函数不同。

**常见坑：**
- 在当前 BFS 层 *进行中* 就删词——会漏掉同层另一个父节点。
- 不在 `end` 首次出现的层停止——在密词典上运行时间爆炸。
...
- 求每个连通分量的 *大小*，而不仅是个数。
- 动态连通性——边是在线增删的。
- *有向* 图的连通分量——改用 Tarjan / Kosaraju SCC。
- 可回滚的并查集，用于离线查询。

**常见坑：**
- 不管两个根是否不同，每次 `union` 都减计数——会多减。
- 同时缺少路径压缩和按秩合并——对抗型输入下每次 find 可能 O(n)。
...
- 重建一个合法 LIS，而不仅是长度。
- LIS 个数（LeetCode 673）。
- 最长 *双调* 子序列——正向 LIS + 反向 LIS。
- 2D LIS / 俄罗斯套娃信封——按一轴排序后在另一轴 LIS。

**常见坑：**
- 把 `tails` 当作真正的 LIS 序列——它不是。
- 非严格递增用了 `bisect_left`——重复值会少计。
...
**常见追问：**
- 三种操作代价不同（插入、删除、替换各有权重）。
- 重建实际编辑脚本，而不仅是距离。
- One Edit Distance——O(n) 判断是否恰好为 1。
- 词粒度编辑距离用于 diff 工具。

**常见坑：**
- 漏了 `dp[i-1][j-1]` 项——只处理了插入/删除，漏了替换。
- 第一行/列初始化为 0 而不是 `i`/`j`——静默错。
...
**常见追问：**
- 通配符匹配 `?` 与 `*`（LeetCode 44）——语义不同，无前项规则。
- 编译正则为 NFA / Thompson 构造。
- 支持 `+` 与 `{n,m}` 量词。
- 返回匹配分组，而不仅 true/false。

**常见坑：**
- 把 `*` 当作通配符——它只重复 *前* 一个字符。
- `a*b*c*` 模式下 `dp[0][j]` 初始化 off-by-one。
...
**复杂度：** 记忆化后有 O(n²) 个不同子问题，但答案集可能指数级，总时间为 O(n² + |输出|)。
...
**复杂度：** O(4ⁿ / √n)——第 n 个卡特兰数个合法串，每个长 2n；递归深度 O(n)。
...
- 全新用户零历史的冷启动。
- 信息茨房 / 回声室问题在排序层的缓解。
- 实时信号：一个"点赞"多快能反馈到下一条推荐？
- 模型训练节奏——小时/日/连续在线学习。
- 推理延迟预算——多任务 DNN 打分怎么在 200ms 内完成？

**常见坑：**
- 混淆候选生成与排序——二者延迟预算差异很大。
- 忽略多样性重排——FYP 坍缩为单一话题。
...
**常见追问：**
- 不稳定移动网络上的可断点续传——分片大小、重试、去重。
- DRM / 水印流水线——放在哪一环？
- 成本：冷 blob vs 热 blob vs CDN——互动衰减后如何分层？
- 审核申诉：误判的视频怎么办？
- 多区域存储放置——主拷贝放哪？

**常见坑：**
- 等全部编码完才允许播放——应在首个分片就绪后就可播。
- 未审核内容进入 FYP 候选池——品牌安全灾难。
...
### 62. 视频编码权衡：H.264 vs H.265 vs AV1
