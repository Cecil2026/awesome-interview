...
**常见追问：**
- 合并 K 个有序链表——表头堆，或两两合并 O(N log K)。
- 链表过大装不下内存——磁盘上的外部合并。
- 双向链表——不重扫的前提下修复 `prev` 指针。
- 不用哑头的原地合并——小心选首节点。

**常见坑：**
- 接上后忘了推进 `tail`——出现自环。
- 忘了 `tail.next = a or b` 接剩余部分——结果被截断。
...
**常见追问：**
- 线程安全——分桶锁 vs 全局锁，讨论争用。
- LFU（最不常用）变体——数据结构不同，淘汰更难。
- LRU 上加一层 TTL 过期。
- 分布式 LRU——一致性哈希 + 每节点本地缓存。

**常见坑：**
- 用普通 dict 扫找最早——`put` 变 O(n)，不是 O(1)。
- `get` 时忘了移位，读不计作近期访问。
...
- 路径必须经过根的变体。
- 推广到 N 叉树或 DAG。
- iOS 坑：深偏斜树冲爆递归栈——提供迭代版本。

**常见坑：**
- 把 `best` 初为 0——全负树会错。
- 给父节点回 `node.val + l + r` 而不是 `node.val + max(l, r)`——产生非法路径。
...
- 计数所有回文子串，而不只是最长。
- 回文划分——把 `s` 拆成最少的回文段。
- 最长回文**子序列**（另一类问题，LCS DP）。
- 苹果可能追问：实现 Manacher 并说明为何 O(n) 重要。
...
**常见追问：**
- 统计凑出的方案数（Coin Change II）——不同的 DP 顺序。
- 每枚硬币最多用一次——经典 0/1 背包。
- 重建一组最优硬币集合，而不只是数量。
- amount 巨大、硬币集小——BFS-on-graph 变体可能更快。

**常见坑：**
- 互换循环顺序会变成统计“组合数”而非“最少硬币数”。
- `dp[amount]` 还是 `INF` 时直接返回——必须重映射为 -1。
...
**常见追问：**
- 对大质数取模代替原始乘积——除法变为模逆元。
- 允许除法，但显式处理 0（计数 + 位置技巧）。
- 2D 版本：除当前格外所有矩阵元素乘积。
- 流式 nums——插入/删除后维护前后缀积。

**常见坑：**
- 用除法遇到 0 元素直接崩。
- 长数组 `int` 乘积溢出——讨论用 `long`/BigInt。
...
**常见追问：**
- 返回所有出现位置——同扫描，匹配时追加。
- 同一 haystack 多模式查找——Aho-Corasick 自动机。
- 通配符 / `?` 模糊匹配——另一类 DP。
- KMP vs Rabin-Karp vs Boyer-Moore——各自什么场景胜出？

**常见坑：**
- `lps` 构造中 off-by-one（`i` 从 1 起，不是 0）。
- 返回的是末位而不是首位（必须 `i - j + 1`）。
...
**复杂度：** O(n²) 计算量——每个元素只动一次，与朴素转置相同；分块只是通过缓存局部性改善常数因子。O(1) 额外空间。
...
**复杂度：** `push` 与 `pop` 均为 O(1) 且无锁；单生产者与单消费者通过两个原子下标协调，无互斥锁。
...
- 计量蜂窝网上的无损流——自适应码率策略。
- 不上传听歌历史的个性化——联邦学习？
- 现场电台 / Apple Music Live——接入 + fanout 怎么变？
- 空间音频元数据管道与解码器协商。
- DRM 许可服务器失败场景——播放如何降级？

**常见坑：**
- 把推荐管道看作单个服务——遗漏了离线/在线拆分。
- 完全不提 DRM——对于音乐服务不现实。
...
**常见追问：**
- 用户新增设备——密钥分发如何处理？
- 大群聊（>100）——sender key vs 两两加密；成员轮换密钥时的失效场景。
- E2E 下的消息编辑/撤回语义——服务端无法强制执行。
- 备份与恢复——iCloud Backup 含密钥，但可选 Advanced Data Protection。
- 不读明文的垃圾/滥用检测——仅元数据信号。
...
### 61. 前端 / web perf：优化 Largest Contentful Paint
