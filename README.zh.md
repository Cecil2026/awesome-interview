...
| [docs/](docs/) | 静态站点——随机抽题工具 + 带语言 tab 切换的 Markdown 阅读器 | 1 |
...
| `/docs/index.html` | 题目选择器 | 按类别 / 难度 / 主题筛选，随机抽题 |
...
三个页面右上角都有 EN / 简体中文 切换和 Light / Dark 主题切换。两个选择都存在 `localStorage`，跨页面和刷新都保留。首次访问的主题默认跟随操作系统偏好（`prefers-color-scheme`）。
...
**冲刺** — 打开 [随机抽题](docs/index.html)（通过 GitHub Pages 从 `docs/` 部署），按难度和主题筛选，直到计时器响。
