...
第一次用？打开 [docs/](docs/)（或运行 `python tools/run_service.py --open`），点击与你当前情况匹配的卡片即可。下面是每种意图的快速对照 —— 每个网页工具都同时给出 CLI / markdown 备选,可按个人习惯选择。
...
**冲刺** — 打开 [随机抽题](docs/picker.html)（通过 GitHub Pages 从 `docs/` 部署），按难度和主题筛选，直到计时器响。针对某公司？在 [对比视图](docs/compare.html) 里并排看各公司的面试形式。手边有简历？把项目经历粘贴到 [简历 → 题目生成器](docs/resume.html)，生成对应题目。
