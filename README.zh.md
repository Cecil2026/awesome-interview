...
第一次用？打开 [docs/](docs/)（或运行 `python tools/run_service.py --open`），点击与你当前情况匹配的卡片即可。下面是每种意图的快速对照表 —— 每个网页工具都同时给出 CLI / markdown 备选方案，可按个人习惯选择。

| 你想做什么 | 用这个 | 或用 CLI / markdown |
|---|---|---|
| 根据目标公司 + 岗位生成多周备考计划 | [Plan](docs/plan.html) | `python tools/daily_plan.py`、[roadmap/](roadmap/) |
| 随手抽一题刷 | [Picker](docs/picker.html) | `python tools/random_pick.py knowledge/` |
| 准备某家公司 | [Compare](docs/compare.html) → [Reader](docs/reader.html) | [interviews/companies/](interviews/companies/) |
| 深入学某个主题 | [Reader](docs/reader.html) | [knowledge/](knowledge/) |
| 基于简历定制面试题 | [Resume → Q](docs/resume.html) | — |
| 记录打卡 / 间隔复习 | — | `python tools/streak.py done`、`python tools/review.py due` |
...
`tools/run_service.py` 启动一个轻量的浏览器服务，让你不用离开终端就能阅读、浏览、刷题。
...
python tools/run_service.py --no-build     # 跳过重新生成 questions.json / md_files.json
python tools/run_service.py --no-kill      # 端口被占时直接报错，不自动 kill 旧进程
```

默认行为：启动前会通过 `netstat`／`lsof` 检测同端口是否已有进程，有就先 kill 再绑定 —— 因此重复运行该命令可以直接生效。加 `--no-kill` 可关闭这个行为。

提供以下页面：

| URL | 页面 | 内容 |
|---|---|---|
| `/` 和 `/docs/index.html` | Start | 意图路由首页 —— 选择与你当前情况匹配的卡片 |
| `/docs/picker.html` | Picker | 按类别／难度／主题筛选、随机抽题、浏览全部题库并跟踪进度 |
| `/docs/reader.html` | Reader | 在干净的 HTML 视图中浏览所有 `*.md` 文件，带侧边栏和目录 |
| `/docs/compare.html` | Compare | 并排对比各公司的轮次、考察重点、语言、时长和难度分布 |
| `/docs/resume.html` | Resume → Q | 粘贴一段简历项目经历，生成对应的技术／行为面试题（完全离线） |
| `/docs/plan.html` | Plan | 根据目标公司、岗位与简历生成多周备考计划（完全离线） |

所有页面共用统一的顶部导航（Start · Picker · Reader · Compare · Resume → Q · Plan），可随时在页面间跳转。

首次运行会通过 `tools/build_index.py` 生成 `docs/questions.json` 和 `docs/md_files.json`，让选择器和阅读器有内容可以渲染。加 `--no-build` 可跳过这一步。

### 语言切换（EN ↔ 中文）与主题切换（Light / Dark）
...
**针对性准备** — 正在面某家具体公司？打开 [interviews/companies/](interviews/companies/) 下对应的文件，里面说明了面试形式、考察重点，每家公司 20+ 道真题。

**冲刺** — 打开 [随机抽题](docs/picker.html)（通过 GitHub Pages 从 `docs/` 部署），按难度和主题筛选，刷到计时器响。针对某家公司？在 [对比视图](docs/compare.html) 里并排查看各公司的面试形式。手边有简历？把项目经历粘贴到 [简历 → 题目生成器](docs/resume.html)，自动生成对应题目。
...
随时添加或修正任何内容。每个文件里的编号只是参考，并非契约 —— 可以自由重新编号。

1. 添加或修改一条 `### N. Question` 条目。
2. 运行 `python tools/build_index.py` 刷新 `docs/questions.json`。
3. 把两个文件一起提交。如果你忘了刷新索引，CI 会拦下这个 PR。
