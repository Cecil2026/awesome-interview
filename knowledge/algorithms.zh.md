...
**问题：** 给定一个未排序的整数数组 nums，返回可以由其元素组成的最长连续整数序列（相邻值恰好相差 1）的长度，与它们在数组中的顺序无关。重复元素只计一次。数组可能为空，长度可达 1 <= n <= 10^5，值可以是 32 位整数范围内的任意值。要求时间复杂度为 O(n)。
...
**问题：** 给定一个整数数组 nums，原地重排使所有 0 移到末尾，同时保持所有非零元素的相对顺序不变。不得额外分配输出数组，并应尽量减少写操作次数。数组长度满足 1 <= n <= 10^4，值为有符号 32 位整数。无返回值，直接修改 nums。
...
**问题：** 给定两个小写字符串 s 和 p，按递增顺序返回 s 中所有作为 p 的字母异位词（p 字符的排列）的子串的起始下标。两个字符串仅由小写英文字母组成，1 <= s.length, p.length <= 3*10^4。若 p 比 s 长，答案为空。输出下标顺序应与其在 s 中的位置一致。
...
**问题：** 给定一个整数数组 nums 和一个整数 k，统计和恰好等于 k 的连续子数组的个数。元素可以为负、为零或为正，因此前缀和不是单调的。数组长度 1 <= n <= 2*10^4，每个值和 k 都在 -10^7 到 10^7 范围内。返回满足条件的子数组总数。
...
**问题：** 给定一个整数数组 nums，将其原地向右旋转 k 个位置，其中 k 非负且可能超过数组长度。每个元素向右移动 k 位，越过末尾的元素绕回到开头。数组长度 1 <= n <= 10^5，0 <= k <= 10^5；应力求使用 O(1) 额外空间的解法。无返回值，直接修改 nums。
...
**问题：** 给定一个未排序的整数数组 nums，找出没有出现在数组中的最小正整数（大于等于 1）。数组可能包含重复元素、负数和零，长度满足 1 <= n <= 10^5，取值覆盖完整的 32 位整数范围。要求在 O(n) 时间内完成，且除输入数组外只使用 O(1) 额外空间。
...
**问题：** 给定一个 m x n 矩阵，每一行从左到右升序排列，每一列从上到下升序排列，判断给定的目标值是否存在于矩阵中并返回布尔结果。维度满足 1 <= m, n <= 300，取值可用有符号 32 位整数表示。注意矩阵并非像展平数组那样全局有序，因此不能对所有单元格做一次简单的二分查找。
...
**问题：** 给定两个可能相交并共享公共尾部的单链表的头节点，返回它们第一次相交的节点；若不相交则返回 null。相交由引用（节点身份）定义，而非值相等，且不能改变链表的原始结构。两链表总长度最多 5 * 10^4 个节点，要求使用 O(1) 额外空间。
...
**问题：** 给定一个单链表的头节点，判断其节点值序列是否正读和倒读相同，返回布尔结果。链表包含 1 到 10^5 个节点，值为较小的非负整数。目标是 O(n) 时间和 O(1) 额外空间，因此构建完整数组或使用递归栈都被视为非最优。
...
**问题：** 给定一个单链表的头节点，两两交换相邻的节点并返回新的头节点，要求改变节点之间的链接而不仅仅是交换存储的值。若链表节点数为奇数，最后剩下的单个节点保持原位。链表包含 0 到 100 个节点；目标为 O(n) 时间和 O(1) 空间。
...
**问题：** 给定一个单链表的头节点，将链表节点按每 k 个一组进行翻转，返回修改后的链表头。如果最后一组不足 k 个节点，则保持这些节点的原有顺序。不能交换节点内部存储的值，只能重排节点之间的链接。链表节点数介于 1 到 5000 之间，每个值介于 0 到 1000 之间，且 1 <= k <= n。
...
**问题：** 给定单链表的头节点，返回按节点值非递减排序后的同一链表。重排必须通过重新链接节点完成，最优解应在 O(n log n) 时间内排序。链表节点数介于 0 到 5 x 10^4 之间，每个值在 32 位有符号整数范围内，即 -10^5 到 10^5。
...
**问题：** 给定二叉树的根节点，返回中序遍历产生的节点值列表，即先遍历左子树，再访问当前节点，最后遍历右子树。树的节点数介于 0 到 100 之间，每个节点值介于 -100 到 100 之间。常见的进阶要求是用迭代而非递归实现。
...
**问题：** 给定二叉树的根节点，判断这棵树是否关于其中心轴对称，若对称返回 true，否则返回 false。对称意味着左子树是右子树的镜像，在结构和节点值上都相互匹配。树的节点数介于 1 到 1000 之间，每个值介于 -100 到 100 之间。
...
**问题：** 给定二叉树的根节点，返回其直径，直径定义为树中任意两个节点之间最长路径上的边数。这条路径不一定经过根节点，且其长度以边数而非节点数计算。树的节点数介于 1 到 10^4 之间，每个节点值介于 -100 到 100 之间。
...
**问题：** 给定一个严格升序排列的整数数组，用它构造一棵高度平衡的二叉搜索树，其中高度平衡指每个节点的两个子树高度差不超过 1。返回任意一棵满足条件的树的根节点。数组长度满足 1 <= n <= 10^4，值可用 32 位有符号整数表示。
...
**问题：** 给定一棵二叉树的根节点，想象你站在它的右侧，返回从上到下你能看到的节点值。换言之，对每一层深度输出其最右侧节点的值。树的节点数在 0 到 100 之间，节点值可用 32 位有符号整数表示。
...
**问题：** 给定一棵二叉树的根节点，将其原地重排为一个「链表」：每个节点的左孩子必须变为 null，右孩子必须指向该树前序遍历中的下一个节点。直接修改原树而非返回新结构。节点数量在 0 到 2000 之间，节点值可用 32 位有符号整数表示。
...
**问题：** 给定一棵二叉树的根节点和一个整数目标值，统计节点值之和等于目标值的向下路径数量，其中路径必须从某个节点沿父到子的方向到达其某个后代（起点终点任意，不必是根到叶）。返回这类路径的总数。树的节点数为 0 到 1000，节点值范围为 [-10^9, 10^9]，目标值为 32 位整数。
...
**问题：** 给定一个 m x n 的网格，每个格子为 0（空）、1（新鲜橘子）或 2（腐烂橘子）。每分钟，任何与腐烂橘子四方向相邻的新鲜橘子都会腐烂。返回直到不存在新鲜橘子所需的最少分钟数；若某个新鲜橘子永远无法腐烂，则返回 -1。网格尺寸满足 1 <= m, n <= 10（格子值为 0、1 或 2）。
...
**问题：** 给定一个只包含小写英文字母的字符串 s，将其切分为若干连续子串，使得每个子串都是回文串，并返回所有可能的切分方案。每个方案是按顺序排列的子串列表，所有方案的整体集合可以以任意顺序返回。约束：1 <= s.length <= 16，因此方案数量虽为指数级但有界。
...
**问题：** 给定一个由不同整数组成的升序数组 nums 和一个目标值 target，如果找到 target 则返回其下标；如果不存在，则返回将其插入后仍保持有序的下标位置。数组严格升序排列。约束：1 <= nums.length <= 10^4，-10^4 <= nums[i], target <= 10^4，且解法必须在 O(log n) 时间内完成。
...
**问题：** 给定一个 m x n 矩阵，其中每一行按升序排列，且每一行的第一个整数都大于上一行的最后一个整数，判断给定的目标值 target 是否存在于矩阵中，返回 true 或 false。由于这种布局，矩阵逐行读取时等价于一个整体有序的序列。约束：1 <= m, n <= 100，-10^4 <= matrix[i][j], target <= 10^4，要求 O(log(m*n)) 的时间复杂度。
...
**问题：** 给定一个使用 k[encoded] 模式的编码字符串，其中方括号内的内容必须恰好重复 k 次，返回完全解码后的字符串。编码可以嵌套（例如 3[a2[c]] 解码为 accaccacc），k 始终为正整数，输入仅包含数字、字母和方括号，没有多余空格或不匹配的括号。约束：1 <= s.length <= 30，1 <= k <= 300，且解码后的输出可以轻松放入内存。
...
**问题：** 给定一个下标从 0 开始的数组 nums，其中每个元素表示从该位置可以向前跳跃的最大长度，返回从下标 0 到达最后一个下标所需的最少跳跃次数。题目保证最后一个下标总是可达。约束：1 <= nums.length <= 10^4，0 <= nums[i] <= 1000。
...
**问题：** 给定一个仅含小写字母的字符串 s。将它划分为尽可能多的连续片段，使得每个不同的字母都只出现在同一个片段中（没有字母跨越两个片段）。按从左到右的顺序返回各片段的长度列表。约束：1 <= s.length <= 500，且 s 仅包含小写英文字母。
...
**问题：** 给定一个整数 numRows，构建帕斯卡三角形（杨辉三角）的前 numRows 行，并以列表的列表形式返回。第 0 行是 [1]；之后每一行都以 1 开头和结尾，且每个内部值等于上一行中位于其左上方与右上方两个值之和。约束：1 <= numRows <= 30。
...
**问题：** 给定一个仅由字符 '(' 和 ')' 组成的字符串 s，求形成格式正确（正确匹配且嵌套）的括号序列的最长连续子串的长度。例如 ")()())" 的答案为 4，对应子串 "()()"。约束：0 <= s.length <= 3 * 10^4。
...
**问题：** 给定一个 m x n 的非负整数网格，找出一条从左上角单元格到右下角单元格、使路径上数字之和最小的路径，每一步只能向右或向下移动。返回这个最小和。约束：1 <= m, n <= 200，且 0 <= grid[i][j] <= 200。
...
**问题：** 给定一个长度为 n 的整数数组，返回出现次数超过 n/2 次的元素。题目保证多数元素一定存在，因此无需做有效性检查。约束条件：1 <= n <= 5*10^4，每个数值都在 32 位有符号整数范围内。
...
**问题：** 给定一个仅包含 0、1、2 三种值的数组，分别代表红、白、蓝三种物体，原地对其排序，使相同颜色的物体相邻并按红、白、蓝的顺序排列。不允许使用库自带的排序函数。约束条件：1 <= n <= 300，每个元素均为 0、1 或 2。
...
**问题：** 给定一个表示排列的整数数组，原地将其重排为下一个字典序更大的排列。如果不存在更大的排列（数组已是降序），则将其变为最小的排列，即升序排序。约束条件：1 <= n <= 100，每个值在 0 到 100 之间。
...
**问题：** 给定一个包含 n+1 个整数的数组，其中每个值都在 1 到 n 的范围内，恰好有一个数字重复出现（可能出现多次），其余数字各出现一次；返回这个重复的数字。不能修改数组，且只能使用常数级额外空间。约束条件：1 <= n <= 10^5，1 <= nums[i] <= n。
