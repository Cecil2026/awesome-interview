...
3-5 年经验的中级后端工程师，目标是 FAANG 级别或高门槛创业公司中 L5 等同的 SWE 岗位。熟练掌握一门后端语言（Go、Java、Python、C++、Rust），已经上线过生产服务，但在大规模系统设计和顶级公司要求的算法门槛上历来偏弱。你在生产环境用过数据库，但也许从未在面试里解释过一份 EXPLAIN 执行计划。
...
**系统设计 / 领域**
- [ ] 写一页总结：何时该选数组、链表、哈希表或有序结构，并附上示例场景
...
**系统设计 / 领域**
- [ ] 写一页笔记，讲清 DP 何时适用（最优子结构 + 重叠子问题）以及如何识别
...
- [ ] 学习：B 树与 LSM 树的对比，各自的使用场景，以及读写放大的取舍
...
**系统设计 / 领域**
- [ ] 写一页笔记：何时用单列索引 vs 复合索引、索引顺序为什么重要、索引何时反而会拖慢性能
...
- [ ] 阅读：任何知名的系统设计入门读物（「System Design Primer」仓库是经典的免费参考）— 略读容量估算部分

**五个设计（每天一个，每个约 3 小时）**
- [ ] 设计一个 URL 短链服务 — 需求、API、base62 编码、哈希冲突、扩展到每天 1 亿 URL、缓存策略、统计分析
- [ ] 设计一个类似 WhatsApp 的聊天系统 — 1:1 与群聊、消息顺序、送达回执、在线状态、推送通知、端到端加密的取舍
...
**每个设计需要产出**
- [ ] 一张白板风格的架构图
- [ ] 一页书面方案：需求 → 估算 → API → 数据模型 → 整体架构 → 1-2 个组件的深入剖析 → 瓶颈与应对方案
...
**重点：** 在算法门槛已经达标之后，把强候选人和普通候选人拉开差距的话题。

**算法（目标：8 题）**
- [ ] 8 道困难题，覆盖你练过的题型。每题限时 40 分钟。

**理论 — 并发**
- [ ] 学习：互斥锁 vs 信号量 vs 读写锁 vs 自旋锁 — 各自的适用场景
- [ ] 学习：死锁 — 四个条件，如何预防或检测
- [ ] 学习：竞态条件、内存序、原子操作
...
- [ ] Python：GIL — 它到底做了什么、什么时候会带来痛点、什么时候不会；asyncio 事件循环；CPython 对象模型
- [ ] C++：移动语义、RAII、智能指针、内存模型（acquire/release）、模板基础
- [ ] Rust：所有权与借用、Send/Sync、async/await 运行时模型、常见生命周期错误

**系统设计 / 领域**
- [ ] 从没做过的题池里再做 1 道系统设计：设计 Uber 派单匹配、设计一个类似 Redis 的分布式缓存、设计一个类似 Kafka 的队列、设计 Dropbox 文件同步

**行为面试准备**
- [ ] 与同伴做 1 次完整行为面试模拟（30-45 分钟）
- [ ] 对 STAR 故事做最后一遍打磨 — 应当听起来轻松且简洁
...
**系统设计 / 领域**
- [ ] 与同伴做 3 次系统设计模拟（每次 45 分钟）— 用你没见过的题目
- [ ] 每次结束后写一页自评：你漏掉了什么、需要加深什么

**行为面试准备**
- [ ] 与同伴或付费服务做 1 次完整流程模拟 — 编码 + 系统设计 + 行为面试端到端
- [ ] 对 STAR 故事做最后审阅
...
- 第 4 周 = 原计划第 7+8 周浓缩。1 次完整流程模拟。死磕最薄弱的算法类别。最终定稿 STAR 故事。

**压缩 2 周版本（应急）：**

- 第 1 周：40 道 LeetCode 中等题，覆盖所有主要题型。只读 DDIA 第 1、5、9 章。1 个系统设计练习（URL 短链）。
- 第 2 周：再做 2 个系统设计（聊天、限流器）。1 次模拟面试。定稿 5 个 STAR 故事。硬件检查。

## 如果时间更多

- 把 DDIA 通读一遍，而不只是指定章节。它是后端面试杠杆率最高的一本书。
- 学一门第二语言。如果你用 Python 面试，就再学到能跟 Go 对比运行时模型的程度（反之亦然）。面试官欣赏广度。
- 实现一个玩具分布式系统：一个带复制和简单共识机制（Raft，或者哪怕是主备）的键值存储。一次动手实现胜过读 10 篇论文。
- 给开源后端项目（数据库、队列、服务网格）提交一个有分量的 PR。一个真实 PR 比任何副业项目都更有说服力。
- 读 5 篇你想加入的团队公开发布的工程博客。记下他们讨论的架构决策和取舍。面试时引用它们。
- 每周研究一份生产事故复盘（Cloudflare、GitHub、AWS、Google 都发过详细的）。学一学真实大规模系统中会出什么问题。
