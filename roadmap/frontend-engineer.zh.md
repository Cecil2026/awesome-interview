...
2-5 年生产经验的中级前端工程师，目标是中大型科技公司的产品工程岗位。你每天都在写 UI 代码，深度使用过至少一个主流框架（React、Vue 或 Angular），并且不用 Google `display: flex` 就能调一个布局 bug。你较弱的方面可能是框架内部原理、压力下的性能审计，以及用面试语言把自己的工作讲清楚。
...
- 你至少用 React、Vue 或 Angular 构建过一个有一定规模的应用
- 你理解 `let`、`const`、`var` 的差异，并能讲清它们为什么重要
- 你用浏览器 DevTools 调试过 JS bug 和 CSS bug
...
**重点：** 重建你对 JavaScript 运行时的心智模型，让框架相关的问题答起来不费力。
...
**系统设计 / 领域**
- [ ] 写一页笔记，讲清 JavaScript 中 `this` 是怎么确定的（4 条规则：默认、隐式、显式、new）
...
**行为面试准备**
- [ ] 从清单里打磨 3 个 STAR 故事（每个 90 秒内讲完情境/任务/行动/结果）
- [ ] 1 个故事展示技术深度，1 个体现协作，1 个讲一次失败或失误
...
**系统设计 / 领域**
- [ ] 在 30 分钟内用 CSS Grid 从零搭一个响应式卡片网格，不用任何框架
- [ ] 搭一个带汉堡菜单的吸顶头部，菜单能打开一个可访问的抽屉（键盘 + 屏幕阅读器友好）
- [ ] 列出 7 个最常见的 ARIA 角色及各自的适用场景
...
**重点：** 选一个，深入到能教别人的程度。面试官 30 秒就能听出浮于表面的框架知识。
...
- [ ] 用 3 分钟讲清你所选框架的核心机制：调和（React）、响应式（Vue）或变更检测（Angular）
...
- [ ] 观看：官方 Chrome for Developers 频道任意一份较新的 Chrome DevTools Performance 面板讲解

**系统设计 / 领域**
- [ ] 用 Lighthouse 审计一个真实的生产网站（你自己的或一个公开站），写一页报告列出前 5 个问题以及你的修复方案
- [ ] 拿一份 webpack/Vite 配置，逐项讲清 — 每个插件做什么、为什么需要它
- [ ] 列出渲染策略：CSR、SSR、SSG、ISR、流式 SSR — 各自的适用场景

**行为面试准备**
- [ ] 准备 3 个针对面试官的问题（要切中该团队，不要泛泛而问）
...
- [ ] 阅读：任一知名备战站点的「前端系统设计」部分（学结构：需求 → 组件树 → 状态 → API → 优化）
...
**系统设计 / 领域**
- [ ] 模拟设计：随机找一个你没见过的题目，独自在纸上 45 分钟做完。例如：设计带懒加载的图片画廊、设计带嵌套回复的评论区、设计富文本编辑器工具栏

**行为面试准备**
- [ ] 与同伴做 1 次行为面试模拟 — 30 分钟，他们问，你冷启动回答
...
**算法（目标：5 题模拟）**
- [ ] 在限时（每题 25 分钟）下解 5 道随机中等题，边讲边写，把思路讲出来

**理论**
- [ ] 不学新材料。把第 1-7 周的笔记重读一遍。

**系统设计 / 领域**
- [ ] 与同伴做 2 次系统设计模拟（各 45 分钟）— 一个聊天 UI 变体，一个信息流变体
- [ ] 每次模拟后写一页自评

**行为面试准备**
- [ ] 与同伴或付费服务做 1 次完整流程模拟 — 编码 + 系统设计 + 行为面试连着做
- [ ] 对 STAR 故事做最后打磨 — 应当听起来轻松自然
...
- [ ] 与朋友做编码模拟面试（45 分钟，没见过的题）
- [ ] 与朋友做系统设计模拟（45 分钟，前端题目）
- [ ] 与朋友做行为模拟（30 分钟，5 个 STAR 问题）
- [ ] 把你最好的 5 个 STAR 故事最后再读一遍
- [ ] 与招聘联系人确认面试形式（轮次、时长、工具、可用语言）
...
- 第 4 周 = 原计划第 7+8 周浓缩。1 次模拟面试，死磕薄弱的算法类别，定稿 STAR 故事。

**压缩 2 周版本（应急）：**

- 第 1 周：30 道 LeetCode 中等题，覆盖数组/字符串/树/图，框架内部原理复习，3 个 STAR 故事。
- 第 2 周：1 个前端系统设计练习，1 次模拟面试，定稿 5 个 STAR 故事，硬件检查。

## 如果时间更多

- 再加一个框架。如果你会 React，就用 Vue 或 Svelte 写一些有分量的东西。面试官喜欢能跨范式比较的候选人。
- 给开源 UI 库（Radix、shadcn、Headless UI、你选用的框架）提交一个有分量的 PR。一个真实 PR 胜过 10 份教程。
- 从头到尾读完 Adam Wathan 与 Steve Schoger 的《Refactoring UI》。前端面试官越来越看重视觉品位。
- 搭一个小型设计系统：按钮、输入、模态框、下拉、表格。把它文档化。这既是作品集又是学习项目。
- 学一点 WebGL 或 Canvas 的基础 — 哪怕只是浅层认知，也能让你在图形密集型产品的面试中脱颖而出。
